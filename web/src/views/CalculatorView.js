                ...
                const membersData = activeMembers.value.map(m => {
                    if (m === 'bebe') {
                        ...
                        return {
                            id: 'ariel',
                            name: 'Ariel (樂樂)',
                            profile: '年輕女性，大一生，體重 57.0kg, BMR 1204 kcal, 骨骼肌 21.0kg, 體脂 32.3% (隱性偏胖增肌減脂型)',
                            mealTarget: '單餐目標：熱量 450~520 kcal, 蛋白質 28~32g (抗炎好蛋白，海鮮/雞肉優先), 碳水 45~55g (以原型低GI粗糧如玉米/地瓜為主), 脂肪 14~18g, 鈉 < 650mg (防浮腫與抗痘)'
                        };
                    } else {
                        return {
                            id: 'jason',
                            ...
                            mealTarget: '單餐目標：熱量 600~720 kcal, 蛋白質 40~48g (大份量肉品/海鮮主菜 120~180g, 蛋可2顆), 碳水 65~80g (地瓜/飯麵約150~200g 提供高代謝充沛活力), 脂肪 16~22g, 鈉 < 700mg (護心血管)'
                        };
                    }
                });
...
【主廚料理與營養精算心法 (Culinary & Nutrition Principles)】：
1. 主從角色與口感存在感 (Role & Presence)：
   - 當選取多種蛋白質（如海鮮+蛋、肉類+豆腐）時，海鮮與肉品應作為主要蛋白質（單份至少給足 60~100g 才有足夠咀嚼感與滿足感），蛋作為輔助蛋白（抓 1 顆即可），切勿為了湊數而把主菜縮小成 10~20g 碎末。
   - 若只有單一蛋白來源（例如只有蛋或只有豆腐），則蛋可給到 2~3 顆以補足蛋白質。
2. 碳水份量的拳頭常識 (Practical Carb Portions)：
   - 根莖類（地瓜/馬鈴薯）與飯麵類應符合日常飲食拳頭大小（單人份地瓜/熟澱粉約 80~120g），避免單餐給到 200g 以上導致總熱量與碳水超載。
3. 蔬菜爆量與高纖 (Fiber & Volume)：
   - 生菜、小黃瓜等葉菜瓜果體積大、熱量低，可大方給予 100~200g 提供高纖與咀嚼飽足感。
4. 油脂與調味恰到好處 (Fats & Seasonings)：
   - 優質油品（橄欖油/酪梨油）抓 5~8g 潤口抗炎，避免高油。
   - 鹽與醬料適量提味，嚴格控鈉。
